...
const { registerUserValidateAsync, loginUserValidateAsync } = require('../models/modelsValidation');
...
router.post('/register', async (req, res) => {
  try {
    ...
    if (emailExists) {
      return res.status(404).json({ message: 'email is already been taken' });
    }
...
router.get('/img/:userId', async (req, res) => {
  try {
    ...
    if (user.img_path) {
      const readStream = fs.createReadStream('../server/resources/avatars/' + user.img_path);
      readStream.pipe(res);
      res.status(200);
    }
  } catch (err) {
